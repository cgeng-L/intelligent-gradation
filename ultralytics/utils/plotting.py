...
class Colors:
    ...
    def __call__(self, i, bgr=False, mask=False):
        ...
        # 随机生成一个颜色
            ...
            return tuple(random_color) if bgr else random_color
        else:
            c = self.palette[int(i) % self.n]
            return (c[2], c[1], c[0]) if bgr else c
...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=True):
        """Plot masks at once.
        Args:
        ...
        rand_colors = torch.rand((masks.shape[0], 3), device=masks.device)
        colors = rand_colors[:, None, None, :]  # shape(n,1,1,3)
        ...
        im_mask_np_resized = np.resize(im_mask_np, (512, 512, 3))
        # self.im[:] = im_mask_np if retina_masks else scale_image(im_mask_np, self.im.shape)
        self.im[:] = im_mask_np_resized if retina_masks else scale_image(im_mask_np_resized, self.im.shape)
...
def output_to_target(output, max_det=1000):

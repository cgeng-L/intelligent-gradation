...
def scale_masks(masks, shape, padding=True):
    ...
    # top, left = (int(round(pad[1] - 0.2)), int(round(pad[0] - 0.2))) if padding else (0, 0)  # y, x
    # bottom, right = (int(round(mh - pad[1] + 0.2)), int(round(mw - pad[0] + 0.2))) #尝试使mask滑移来解决边缘问题(其实用retina_masks已经解决)
